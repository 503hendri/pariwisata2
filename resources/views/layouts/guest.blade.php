...
<head>
    ...
    <meta name="description"
        content="{{ $websiteProfile->description ?? 'Sawahlunto Tourism - Official Digital Tourism Portal for Sawahlunto, UNESCO World Heritage Ombilin Coal Mining Heritage. Where History Meets Natural Beauty.' }}">
    <meta name="keywords"
    ...
    <meta property="og:title" content="{{ $websiteProfile->name ?? 'Sawahlunto Tourism' }} | UNESCO World Heritage">
    <meta property="og:description"
        content="{{ $websiteProfile->description ?? 'Discover the historic mining city of Sawahlunto, a UNESCO World Heritage Site.' }}">
    <meta property="og:image" content="{{ $websiteProfile?->cover ? Storage::url($websiteProfile->cover) : 'https://picsum.photos/id/1015/1200/630' }}">
    <meta property="og:url" content="{{ config('app.url') }}">

    <title>
        {{ filled($title ?? null) ? $title . ' - ' . ($websiteProfile->name ?? config('app.name', 'Laravel')) : ($websiteProfile->name ?? config('app.name', 'Laravel')) }}
    </title>

    @if ($websiteProfile?->favicon)
        <link rel="icon" href="{{ Storage::url($websiteProfile->favicon) }}">
    @else
...
<body class="tail-container bg-[#F8F6F1] text-[#1C1C1C] overflow-x-hidden">
    <nav class="fixed top-0 left-0 right-0 z-50 glass border-b border-white/30" x-data="{ open: false }">
        <div class="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
            @php
                $logoUrl = $websiteProfile?->logo ? Storage::url($websiteProfile->logo) : Storage::url('images/sawahlunto_tourism.png');
                $brandName = $websiteProfile->name ?? 'Sawahlunto';
            @endphp
    ...
    @php
        $footerName = $websiteProfile->name ?? 'Sawahlunto Tourism';
        $footerDesc = $websiteProfile->description ?? 'Portal resmi pariwisata Kota Sawah Lunto. Jelajahi warisan tambang batubara, budaya kolonial, dan kuliner autentik di jantung Sumatera Barat.';
        $footerAddress = $websiteProfile->address ?? 'Jl. Kebun Jati No. 1 Saringan';
        $footerPhone = $websiteProfile->phone ?? '+62 821 8024 6567';
        $footerEmail = $websiteProfile->email ?? 'info@sawahluntotourism.sawahluntokota.go.id';
        $footerFacebook = $websiteProfile->facebook ?? null;
        $footerInstagram = $websiteProfile->instagram ?? null;
        $footerTiktok = $websiteProfile->tiktok ?? null;
        $footerYoutube = $websiteProfile->youtube ?? null;
    @endphp

    <footer id="contact" class="bg-zinc-900 text-stone-300 pt-16 pb-8 border-t border-zinc-800">
                ...
                <div class="flex justify-center gap-4">
                    @if ($footerFacebook)
                        <a href="{{ $footerFacebook }}" target="_blank" rel="noopener"
                            class="w-12 h-12 rounded-full bg-zinc-800 flex items-center justify-center hover:bg-blue-600 transition duration-300 group">
                            <svg class="w-6 h-6 text-stone-400 group-hover:text-white" fill="currentColor"
                                viewBox="0 0 24 24">
                                <path
                    ...
                    @if ($footerInstagram)
                        <a href="{{ $footerInstagram }}" target="_blank" rel="noopener"
                            class="w-12 h-12 rounded-full bg-zinc-800 flex items-center justify-center hover:bg-pink-600 transition duration-300 group">
                            <svg class="w-6 h-6 text-stone-400 group-hover:text-white" fill="currentColor"
                                viewBox="0 0 24 24">
                                <path
                    ...
                    @endif
                    @if ($footerTiktok)
                        <a href="{{ $footerTiktok }}" target="_blank" rel="noopener"
                            class="w-12 h-12 rounded-full bg-zinc-800 flex items-center justify-center hover:bg-black border border-zinc-700 hover:border-white transition duration-300 group">
                            <svg class="w-6 h-6 text-stone-400 group-hover:text-white" fill="currentColor"
                                viewBox="0 0 24 24">
                                <path
                    ...
                    @if ($footerYoutube)
                        <a href="{{ $footerYoutube }}" target="_blank" rel="noopener"
                            class="w-12 h-12 rounded-full bg-zinc-800 flex items-center justify-center hover:bg-red-600 transition duration-300 group">
                            <svg class="w-6 h-6 text-stone-400 group-hover:text-white" fill="currentColor"
                                viewBox="0 0 24 24">
                                <path
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 mb-12">
                ...
                <div class="space-y-6">
                    <div class="flex items-center gap-2">
                        {{-- <div
                            class="w-10 h-10 bg-green-700 rounded-lg flex items-center justify-center text-white font-bold text-lg">
                            SL</div> --}}
                        <span class="font-bold text-xl text-white tracking-wide">{{ $footerName }}</span>
                    </div>
                    ...
                    <div class="flex space-x-3">
                        @if ($footerFacebook)
                            <a href="{{ $footerFacebook }}" target="_blank" rel="noopener"
                                class="w-9 h-9 rounded-lg bg-zinc-800 flex items-center justify-center hover:bg-blue-600 transition duration-300">
                        ...
                        @if ($footerTiktok)
                            <a href="{{ $footerTiktok }}" target="_blank" rel="noopener"
                                class="w-9 h-9 rounded-lg bg-zinc-800 flex items-center justify-center hover:bg-black border border-zinc-700 hover:border-white transition duration-300">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                                        d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z" />
                                </svg>
                        ...
                        @if ($footerInstagram)
                            <a href="{{ $footerInstagram }}" target="_blank" rel="noopener"
                                class="w-9 h-9 rounded-lg bg-zinc-800 flex items-center justify-center hover:bg-pink-600 transition duration-300">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                                        d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                                </svg>
                        ...
                        @if ($footerYoutube)
                            <a href="{{ $footerYoutube }}" target="_blank" rel="noopener"
                                class="w-9 h-9 rounded-lg bg-zinc-800 flex items-center justify-center hover:bg-red-600 transition duration-300">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                    <path
                                        d="M19.615 3.184c-3.604-.246-11.631-.245-15.23 0-3.897.266-4.356 2.62-4.385 8.816.029 6.185.484 8.549 4.385 8.816 3.6.245 11.626.246 15.23 0 3.897-.266 4.356-2.62 4.385-8.816-.029-6.185-.484-8.549-4.385-8.816zm-10.615 12.816v-8l8 3.993-8 4.007z" />
                                </svg>
                ...
                <div id="footer-contact-info">
                    ...
                    <ul class="space-y-4 text-sm">
                        <li class="flex items-start gap-3">
                            ...
                            <span class="text-stone-400">{{ $footerAddress }}</span>
                        </li>
                        @if ($footerPhone)
                            <li class="flex items-center gap-3">
                                ...
                                <a href="tel:{{ preg_replace('/[^0-9+]/', '', $footerPhone) }}" class="text-stone-400 hover:text-white transition">{{ $footerPhone }}</a>
                            </li>
                        @endif
                        @if ($footerEmail)
                            <li class="flex items-center gap-3">
                                ...
                                <a href="mailto:{{ $footerEmail }}" class="text-stone-400 text-xs hover:text-white transition break-all">{{ $footerEmail }}</a>
                            </li>
                        @endif
                        {{-- <li class="flex items-start gap-3">
                            <span
